...
class ValidateGitURL(argparse.Action):
    ...
    def __call__(self, parser, args, values, option_string=None):
        if not values.startswith('https://github.com/') or not values.endswith(
                '.git'):
            logging.error('Exception occurred.  ValidateGitURL.')
            raise argparse.ArgumentError(
                self, '''Something wend wrong. Is your path correct?\n
...
class ValidatePositiveInt(argparse.Action):
    ...
    def __call__(self, parser, args, values, option_string=None):
        if values <= 0:
            ...
            raise argparse.ArgumentError(
                self, '''Something wend wrong. Is your number is correct?\n
                It should be > 0.\n''')
        setattr(args, self.dest, values)
